...
## Design System
The brand is **Pocketbud** ("Your AI budtender. Lives in your pocket, never leaves it.") — leading name as of 2026-05-30, repo/target rename still deferred. Read `DESIGN.md` before any visual or UI decision. It defines the **green-primary / amber-accent** palette, SF Pro Rounded typography, dark-green-ink app surfaces, the **three privacy pillars** pattern, and the app icon (a white-shirt **pocket protector** with a green leaf + lit joint popping out — source `docs/branding/pocketbud-icon.svg`, master `docs/branding/icon-master-1024.png`). The live mockup of all four screens is `docs/branding/showcase.html`. The earlier amber-forward look and the older green→violet gradient (e.g. in `Brand.swift`/`OnboardingView.swift`) are **superseded** — reskin to the DESIGN.md palette when touching UI. App Store requirements (cannabis guideline 1.4.3, "No Data Collected" label) are in `docs/APP-STORE-CHECKLIST.md`. Do not deviate from DESIGN.md without explicit user approval.
